/*
CONSTRUINDO UM COMPONENTE DINÂMICO (reutilizável), DO TIPO CLASSE: >>> PROPS (propriedades, PARAMETRIZAÇÃO) <<<<
...
class App extends React.Component{
    ...
    render() {
        return(  <localizamos as props aqui> {CHAMAMOS AS PROPS AQUI} </>  );
  }
...
class App2 extends React.Component{     
    constructor(props){     //para parametrizar o componente, chamamos as "props" com o constructor.
        super(props);
    }

    /*este componente terá 3 propriedades: título, texto children. (children é uma prop pré-definida?)

    Como se lê: {this.props.titulo} ?

    this = deste componente
    this.props = propriedades deste componente
    this.props.titulo = valor da propriedade -titulo- deste componente
    */

    render(){          
        return(         
        <div className="caixa">             
            <div className="titulo">{this.props.titulo}</div>  {/*neste momento, criamos a props titulo, parametrizamos o texto html que viria aqui*/}
            <div className="texto">{this.props.texto}</div>
            {/*se o texto for grande, pode usar a abordagem childen:*/}
            <div className="textoG">{this.props.children}</div>
        </div>);
    }
}
...
// ?Onde está o conteúdo (valores) referenciado no className? No index.js
